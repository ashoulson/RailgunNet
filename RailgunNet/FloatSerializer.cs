/*
 *  RailgunNet - A Client/Server Network State-Synchronization Layer for Games
 *  Copyright (c) 2016 - Alexander Shoulson - http://ashoulson.com
 *
 *  This software is provided 'as-is', without any express or implied
 *  warranty. In no event will the authors be held liable for any damages
 *  arising from the use of this software.
 *  Permission is granted to anyone to use this software for any purpose,
 *  including commercial applications, and to alter it and redistribute it
 *  freely, subject to the following restrictions:
 *  
 *  1. The origin of this software must not be misrepresented; you must not
 *     claim that you wrote the original software. If you use this software
 *     in a product, an acknowledgment in the product documentation would be
 *     appreciated but is not required.
 *  2. Altered source versions must be plainly marked as such, and must not be
 *     misrepresented as being the original software.
 *  3. This notice may not be removed or altered from any source distribution.
*/

using System;
using System.Collections.Generic;

using UnityEngine;

namespace RailgunNet
{
  /// <summary>
  /// Compresses floats to a given range with a given precision.
  /// http://stackoverflow.com/questions/8382629/compress-floating-point-numbers-with-specified-range-and-precision
  /// </summary>
  public class FloatSerializer
  {
    public const float MINIMUM_PRECISION = 0.0001f;

    private readonly float precision;
    private readonly float invPrecision;

    private readonly int minValueInt;
    private readonly int maxValueInt;

    private readonly float minValueFloat;
    private readonly float maxValueFloat;

    private readonly int requiredBits;
    private readonly uint mask;

    public int RequiredBits { get { return this.requiredBits; } }

    // http://stackoverflow.com/questions/15967240/fastest-implementation-of-log2int-and-log2float
    private static readonly int[] DeBruijnLookup = new int[32]
    {
        0, 9, 1, 10, 13, 21, 2, 29, 11, 14, 16, 18, 22, 25, 3, 30,
        8, 12, 20, 28, 15, 17, 24, 7, 19, 27, 23, 6, 26, 5, 4, 31
    };

    private static int Log2(int v)
    {
      v |= v >> 1; // Round down to one less than a power of 2 
      v |= v >> 2;
      v |= v >> 4;
      v |= v >> 8;
      v |= v >> 16;

      return DeBruijnLookup[(uint)(v * 0x07C4ACDDU) >> 27];
    }

    /// <summary>
    /// Initializes a float serializer. We use shorts to retrieve the min and
    /// max values in order to avoid overflow errors during conversion.
    /// </summary>
    public FloatSerializer(float precision, short minValue, short maxValue)
    {
      if (precision < FloatSerializer.MINIMUM_PRECISION)
      {
        Debug.LogWarning("Precision too small, adjusting: " + precision);
        precision = FloatSerializer.MINIMUM_PRECISION;
      }

      this.precision = precision;
      this.invPrecision = 1.0f / precision;

      this.minValueInt = minValue;
      this.maxValueInt = maxValue;

      this.minValueFloat = minValue;
      this.maxValueFloat = maxValue;

      this.requiredBits = this.ComputeRequiredBits();
      this.mask = (uint)((1L << requiredBits) - 1);
    }

    public uint Pack(float value)
    {
      value = Mathf.Clamp(value, this.minValueFloat, this.maxValueFloat);
      float adjusted = (value - this.minValueFloat) * this.invPrecision;
      return (uint)(adjusted + 0.5f) & this.mask;
    }

    public float Unpack(uint value)
    {
      float adjusted = ((float)value * this.precision) + this.minValueFloat;
      return Mathf.Clamp(adjusted, this.minValueFloat, this.maxValueFloat);
    }

    private int ComputeRequiredBits()
    {
      int range = this.maxValueInt - this.minValueInt;
      float maxVal = range * (1.0f / this.precision);
      return FloatSerializer.Log2((int)maxVal) + 1;
    }

    #region Debug
    public void Test(int iterations)
    {
      short min = (short)UnityEngine.Random.Range(short.MinValue, 0);
      short max = (short)UnityEngine.Random.Range(0, short.MaxValue);
      float precision =
        UnityEngine.Random.Range(FloatSerializer.MINIMUM_PRECISION, 1.0f);
      FloatSerializer serializer = new FloatSerializer(precision, min, max);

      for (int j = 0; j < iterations; j++)
      {
        float random = UnityEngine.Random.Range(min, max);
        uint packed = serializer.Pack(random);
        float unpacked = serializer.Unpack(packed);

        if (Mathf.Abs(random - unpacked) > precision)
          Debug.LogWarning(
            random +
            " " +
            unpacked +
            " " +
            Mathf.Abs(random - unpacked));
      }
    }
    #endregion
  }
}